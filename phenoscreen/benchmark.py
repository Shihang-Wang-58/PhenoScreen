...
def load_molecular_representation(model_name):
    ...
    if os.path.exists(f'{model_name}/GCMol.pt'):
        from model.GeminiMol import GeminiMol
        from model.gcmol import GCMol

        mol_encoder_framework = GeminiMol(
            encoding_method,
            custom_label = None, 
            extrnal_label_list = [
                'Cosine', 'Pearson', 'RMSE', 'Manhattan'
            ])
        predictor = GCMol(
            model_name, 
            # mol_encoder_framework,
...
class Benchmark():
    ...
    def PropDecoder(
            self,
            target_list=None, 
            candidate_smiles_columns=['SMILES', 'Drug', 'smiles', 'compound', 'molecule', 'mol','smi'], 
            candidate_label_columns=['Label', 'label', 'Y', 'Target', 'target', 'p_np', 'Class', 'measured log solubility in mols per litre', 'expt', 'exp', 'SR-p53'], 
            standardize=False, 
        ):
    ...
    def FineTuning(
            ...
            target_list=None, 
            candidate_smiles_columns=['SMILES', 'Drug', 'smiles', 'compound', 'molecule', 'mol','smi'], 
            candidate_label_columns=['Label', 'label', 'Y', 'Target', 'target', 'p_np', 'Class', 'measured log solubility in mols per litre', 'expt', 'exp', 'SR-p53','FDA','logSolubility','freesolv','HIV_active','lipo','u0_atom'], 
            standardize=False, 
        ):
        from FineTuning_combine import GeminiMolQSAR
        del self.predictor
        self.target_list = target_list
        # benchmark_results (targets, metrics)
        benchmark_results = pd.DataFrame(columns=['model', 'model_score', 'test_metrics'], index=self.target_list)
        for target in self.target_list:
            ...
            for candidate_label_column in candidate_label_columns:
                if candidate_label_column in training_data.columns:
                    ...
                    break
            label_set = list(set(training_data[label_column].to_list()))
            # hyperparameters tuning for the QSAR model
            if len(label_set) == 2:
                task_type = 'binary'
                test_metrics = 'AUROC'
                benchmark_task_type = 'classification'
            ...
            if not os.path.exists(f"{self.model_name}/{self.benchmark_name}/{target}/predictor.pt"):
                ...
                if len(training_data) > 30000:
                    ...
                    expand_ratio, hidden_dim, num_layers = 0, 1024, 3
                if task_type == 'binary':
                    dropout_rate = 0.3 # 0.0, 0.3 no
                    dense_dropout = 0.1 # 0.0, 0.1 yes
                    dense_activation = 'GELU' # Softplus, GELU
                    projection_activation = 'GELU' # Softplus, GELU
                    projection_transform = 'Sigmoid'
                elif task_type == 'regression':
                    dropout_rate = 0.0 # 0.1
                    dense_dropout = 0.0
                    dense_activation = 'ELU' # ELU
                    projection_activation = 'Identity' # Identity, ELU
                    if training_data[label_column].max() <= 1.0 and training_data[label_column].min() >= 0.0:
                        projection_transform = 'Sigmoid'
                    else:
                        projection_transform = 'Identity'
             ...
             # 生成和保存预测结果
            predictions_df = QSAR_model.predict(test_data)
            predictions_df.to_csv(f"{self.model_name}/{self.benchmark_name}/{target}_predictions.csv", index=False)
    ...
    def __call__(self, benchmark_name, data_path, standardize=False):
        ...
        benchmark_functions = {
            ...
            'FineTuning-QSAR_PCBA': partial(
                self.FineTuning,
                target_list=[
                "ALDH1", "FEN1", "GBA", "MAPK1", "PKM2", "KAT2A", "VDR" # "ADRB2", "ALDH1", "ESR1_ago", "ESR1_ant", "FEN1", "GBA", "IDH1", "KAT2A", "MAPK1", "MTORC1", "OPRK1", "PKM2", "PPARG", "TP53", "VDR"
                ], 
                standardize=standardize,
            ),
            'ADMET-C': partial(
            ...
            'MN-QSAR': partial(
                self.QSAR, 
                target_list=[
                    "bace", "bbbp", "clintox", "sider" , "tox21", "toxcast", "hiv", "muv", "esol", "freesolv", "lipophilicity", "qm7", "qm8", "qm9"
                ], 
                smiles_column='smi', 
...
if __name__ == '__main__':
    ...
    if os.path.exists(f'{model_name}/GCMol.pt'):
        from model.GeminiMol import GeminiMol
        # from model.gcmol import GCMol
        # from model.gcmol_vit import GCMol
        
        # from model.gcmol_vit_gate_1024_combine_M2490 import GCMol
        from model.gcmol import GCMol
        # from model.gcmol_vit_1024_combine_M2490 import GCMol
        # from model.gcmol_vit_1024_combine_M5086 import GCMol
        
        # from model.gcmol_Gate import GCMol
        # from model.GCMol_ori import GCMol
        mol_encoder_framework = GeminiMol(
            encoding_method,
            custom_label = None, 
            extrnal_label_list = [
