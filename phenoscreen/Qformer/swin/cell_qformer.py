...
class NearCellCoordsGradient(torch.autograd.Function):
    @staticmethod
    def forward(ctx, coords, centers, coords_lambda=1):
        ctx.in1 = coords_lambda
        ctx.in2 = centers
        ctx.save_for_backward(coords)
        return coords
    ...
    def backward(ctx, grad_output):
        ...
        cell_nums = centers.shape[1]
        reshaped_coords = rearrange(coords, '(b h) wh ww c -> b h wh ww c', b=batch_size)
        reshaped_grad = torch.zeros_like(reshaped_coords)

        # Repeat reshaped_coords to match the number of centers
        repeated_reshaped_coords = reshaped_coords.unsqueeze(4).repeat(1,1,1,1,cell_nums,1)

        # Expand centers to match the dimensions of reshaped_coords
        centers_expanded = centers.unsqueeze(1).unsqueeze(1).unsqueeze(1).repeat(1,*(reshaped_coords.shape[1:4]),1,1)
        
        differences = repeated_reshaped_coords - centers_expanded
        distances = torch.sum(differences**2, dim=-1)
        try:
            min_distances, min_indices = torch.min(distances, dim=-1)
        except IndexError:
            print("distances' shape", distances.shape)
            print("centers_expanded' shape", centers_expanded.shape)
            print("repeated_reshaped_coords' shape", repeated_reshaped_coords.shape)
            print("reshaped_coords' shape", reshaped_coords.shape)
            print("centers' shape", centers.shape)
            print("differences' shape", differences.shape)
            print("centers", centers)
            raise IndexError
        min_indices = min_indices.unsqueeze(-1).unsqueeze(-1).repeat(1,1,1,1,1,2)
        # Extract the closest centers using the indices
        closest_centers = torch.gather(centers_expanded, dim=4, index=min_indices).squeeze(-2)
        # print("closest_centers' shape", closest_centers.shape)

        vector_to_centers = closest_centers - reshaped_coords
        far_mask = min_distances > 0.25 # 0.5 ** 2
        grad_correction = vector_to_centers * (coords_lambda * 10)
        reshaped_grad[far_mask] += grad_correction[far_mask]
        reshaped_grad = torch.clamp(reshaped_grad, min = -coords_lambda*10, max=coords_lambda*10)
        grad_output += rearrange(reshaped_grad, 'b h wh ww c -> (b h) wh ww c')
...
class QuadrangleAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., window_size=7, rpe='v2', coords_lambda=20):
        ...
        if self.emb == '2d':
            self.centeremb_xy = nn.Sequential(
                nn.Embedding(128**2, dim, max_norm=1),
                nn.LayerNorm(dim)
            )
        elif self.emb == '0d':
            self.centerlinearemb = nn.Sequential(
                nn.Linear(2, dim),
                nn.LayerNorm(dim)
            )
        self.transform_heads = 2
        self.transform_scale = (dim // self.transform_heads) ** -0.5
        # self.x_self_attn = nn.MultiheadAttention(embed_dim=dim, num_heads=self.transform_heads, batch_first=True)
        # self.center_self_attn = nn.MultiheadAttention(embed_dim=dim, num_heads=self.transform_heads, batch_first=True)
        self.cross_attn = nn.MultiheadAttention(embed_dim=dim, num_heads=self.transform_heads, batch_first=True)
        self.avgpool = nn.AvgPool2d(kernel_size=self.window_size, stride=self.window_size)
        self.leakyrelu = nn.LeakyReLU()
        self.conv = nn.Conv2d(dim, self.num_heads*9, kernel_size=1, stride=1)
        if rpe == 'v1':
            # define a parameter table of relative position bias
            self.relative_position_bias_table = nn.Parameter(
    ...
    def transform_pro(self, x, centers):
        x = self.avgpool(x)
        x = self.leakyrelu(x)
        stored_h, stored_w = x.shape[2:]
        x = rearrange(x, 'b c h w -> b (h w) c')
        # key_padding_mask = (centers[:, :, 0] == -2)
        if self.emb == '2d':
            centers_1d = (centers[:,:,0] / 4).int() * 128 + (centers[:,:,1] / 4).int()
            center_embedding = self.centeremb_xy(centers_1d)
        elif self.emb == '0d':
            center_embedding = self.centerlinearemb(centers)

        # center_out, _ = self.center_self_attn(center_embedding * self.transform_scale, center_embedding, center_embedding)
        # x_self_out, _ = self.x_self_attn(x * self.transform_scale, x, x)
        # x_cross_out, _ = self.cross_attn(x_self_out * self.transform_scale, center_out, center_out)
        # x_combined = x_self_out + x_cross_out
        x_combined, _ = self.cross_attn(self.norm(x) * self.transform_scale, center_embedding, center_embedding)
        x_combined = self.leakyrelu(x_combined)
        x_combined = x_combined + x
        x_combined = rearrange(x_combined, 'b (h w) c -> b c h w', h=stored_h, w=stored_w)

        final_output = self.conv(x_combined)
        return final_output



    def forward(self, x, h, w, centers):
        b, N, C = x.shape
        ...
        if h > ws or w > ws:
            ...
            # answer = self.transform(x).reshape(b*self.num_heads//self.window_num, 9, window_num_h, window_num_w).permute(0, 2, 3, 1)
            sampling_ = self.transform_pro(x, centers).reshape(b*self.num_heads//self.window_num, 9, window_num_h, window_num_w).permute(0, 2, 3, 1)
            # assert answer.shape == sampling_.shape
            ...
            # answer_offsets = answer[..., :2,]
            # answer_offsets[..., 0] = answer_offsets[..., 0] / (expand_w // ws)
            # answer_offsets[..., 1] = answer_offsets[..., 1] / (expand_h // ws)
            # assert sampling_offsets.shape == answer_offsets.shape
            ...
            # sample_coords = NearCellCoordsGradient.apply(sample_coords, centers, self.coords_lambda)
    ...
    def forward(self, x, centers):
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, centers)
            else:
                x = blk(x, centers)
...
class CellQFormer(nn.Module):
    ...
    Args:
        ...
        in_chans (int): Number of input image channels. Default: 5
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=5, num_classes=1000,
    ...
    def forward_features(self, x, centers):
    ...
    def forward(self, x, centers):
        x = self.forward_features(x, centers)
        x = self.head(x)
        return x
