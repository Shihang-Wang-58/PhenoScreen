...
class QuadrangleAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., window_size=7, rpe='v2', coords_lambda=20):
        ...
        if self.emb == '2d':
            self.centeremb_xy = nn.Embedding(128**2, dim, max_norm=1)
        elif self.emb == '1d':
            self.centeremb_x = nn.Embedding(512, int(dim / 2), max_norm=1)
            self.centeremb_y = nn.Embedding(512, int((dim + 1) / 2), max_norm=1)
    ...
    def transform_pro(self, x, centers):
        ...
        if self.emb == '2d':
            ...
            center_embedding = torch.cat([self.centeremb_x(centers[:,:,0].int()), self.centeremb_y(centers[:,:,1].int())], dim=-1)
        ...
        x_combined, _ = self.cross_attn(x * self.transform_scale, center_embedding, center_embedding)
...
class CellQFormer_Encoder(nn.Module):
